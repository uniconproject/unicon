...
modern multiprocessor and multicore systems, the harware is designed
...
such as [1] or [2] for an overview. Since Unicon{\textquoteright}s
...
sequential. Only one
co-expression is active at any given moment. When a co-expression
is activated, the calling co-expression blocks until the child
co-expression returns the execution to it or fails. Threads on the
other hand,
can run simultaneously and independently. Threads in Unicon
are as special co-expressions that are marked to run asynchronously.
...
Unicon Programs start execution in the
{\textquotedblleft}main{\textquotedblright} procedure. In a
multi-thread programming environment, the procedure main is
the entry point for a special thread referred to as the
{\textquotedblleft}main{\textquotedblright} thread. This main thread
...
\textstyleCodeStyleChar{thread} reserved word or using the function
\textstyleCodeChar{spawn()}. The difference between the two is the
...
\textstyleCodeStyleChar{spawn()} however takes a previously created
co-expression and turns it into a thread. In many cases the thread
reserved word allows more concise code. \textstyleCodeChar{spawn()} on
the other hand is useful in situations where several threads need to be
created and initialized before running them.
\textstyleCodeStyleChar{spawn()} also takes extra optional parameters
...
Both thread and \textstyleCodeChar{spawn()} return a reference to the
...
\textstyleCodeChar{{\textquotedblleft}}\textstyleCodeChar{main}\textstyleCodeChar{
}\textstyleCodeChar{:}\textstyleCodeChar{
}\textstyleCodeChar{done}\textstyleCodeChar{{\textquotedblright}}\textstyleCodeChar{.}
...
using the function \textstyleCodeChar{wait()}, which blocks the calling
...
\textstyleCodeStyleChar{wait(!L)} forces the main the thread to wait for
...
program ends. \textstyleCodeStyleChar{wait()} is very useful in cases
where threads need to synchronize so that one thread blocks until
another finishes. \textstyleCodeStyleChar{wait()} provides a very basic
...
running it in the case of \textstyleCodeChar{spawn()}. Another
...
Instead of using three variables \textstyleCodeChar{x},
\textstyleCodeChar{y} and \textstyleCodeChar{z}, a list of size three
is used. \textstyleCodeChar{x} from the previous example maps to
\textstyleCodeChar{L[1]}, \textstyleCodeChar{y} maps to
\textstyleCodeChar{L[2]}, and z maps to \textstyleCodeChar{L[3]}. The
program does the same thing as before, the difference is that any
change to the content of \textstyleCodeChar{L} is visible in other
threads. Unlike the output in the first case where the values of
\textstyleCodeChar{x}, \textstyleCodeChar{y}, and \textstyleCodeChar{z}
...
increment a shared variable called \textstyleCodeChar{x} whose initial
value is \textstyleCodeChar{0}.
...
While \textstyleCodeChar{x:=x+1} does not look like it would make a
problem, in reality it does. This happens because
\textstyleCodeChar{x:=x+1} is not atomic. In many computer systems this
can be broken down to three operations: fetch the value of
\textstyleCodeChar{x}, increment \textstyleCodeChar{x} and then store
the new value back in \textstyleCodeStyleChar{x}. These three
operations might occur at different times in different threads.
\textit{Thread}\textit{ }\textit{2} for example might fetch
\textstyleCodeStyleChar{x}, followed by \textit{Thread}\textit{
}\textit{1} also fetching it but before \textit{Thread 2} stores back
the new value of \textstyleCodeStyleChar{x}, leaving \textit{Thread 1
}working on the old value of \textstyleCodeStyleChar{x}. \textit{Thread
1 }should not be allowed to read the value of
\textstyleCodeStyleChar{x} in the middle of an operation where another
thread is updating the value of the variable. \ Consider the following
scenarios, starting with \ \textstyleCodeChar{x:=0}\textstyleCodeChar{
}
...
\ \ \ {}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-
The final value of \textstyleCodeStyleChar{x} is \textstyleCodeChar{1}
...
\ \ \ {}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-
The final value of \textstyleCodeStyleChar{x} is \textstyleCodeChar{2}


\bigskip

In Scenario 1 the final value of \textstyleCodeChar{x} is
\textstyleCodeChar{1}, even though there are two increments done by the
two threads. In Scenario 2 however the final value is
\textstyleCodeChar{2}. This unpredictable outcome does not necessarily
...
\textstyleCodeChar{wait()} function or \textstyleCodeChar{delay()}. The
\textstyleCodeChar{wait()} function is the best solution for this
situation. The function \textstyleCodeChar{delay() }works but there are
...
\textstyleCodeChar{delay()} would be a poor choice. Here is an
alternative solution other than using \textstyleCodeChar{wait()}:
...
In this case, the loop \textstyleCodeChar{until
{\textbackslash}}\textstyleCodeChar{done} ensures that the main thread
keeps spinning until the child thread set the variable
\textstyleCodeChar{done} to a non-null value. It avoids the problems
with using delay(), at the expense of burning up 100\% of one CPU in a
spin-lock. Note that declaring \textstyleCodeChar{done} to be global is
...
\textstyleCodeChar{done} in the child thread would be invisible in the
...
access to the variable \textstyleCodeChar{x}. Any operation (including
...
A mutex object is created using the \textstyleCodeChar{mutex()}
function. The returned mutex object can be locked/unlocked via the
functions \textstyleCodeChar{lock()} and \textstyleCodeChar{unlock()}
...
(\textstyleCodeStyleChar{t1} and \textstyleCodeStyleChar{t2}) accessing
the critical region (\textstyleCodeStyleChar{x := x + 1} ).
\textstyleCodeStyleChar{lock(region)} marks the beginning of the
critical region protected by the mutex region.
\textstyleCodeStyleChar{unlock(region)} marks the end of the critical
region. When a thread calls \textstyleCodeStyleChar{lock(region)}, it
tries to acquire the mutex \textstyleCodeStyleChar{region}. If
\textstyleCodeStyleChar{region} is not
{\textquotedblleft}owned{\textquotedblright} by any other thread,
\textstyleCodeStyleChar{lock(region)} succeeds and the thread becomes
the owner of the mutex \textstyleCodeStyleChar{region} and then enters
...
\textstyleCodeStyleChar{unlock(region)}. Since there are two threads
and \textstyleCodeChar{x:=x+1} is protected by a mutex, the output of
the program is guaranteed to be \textstyleCodeChar{x=2}, unlike the
case where a mutex is not used, in which case \textstyleCodeChar{x=1}
or \textstyleCodeChar{x=2} are viable outputs.
...
equivalent to a \textstyleCodeChar{lock()}/\textstyleCodeChar{unlock()}
...
Given a global variable named \textstyleCodeStyleChar{region} that has
been initialized as a mutex, the code to increment
\textstyleCodeChar{x} in the previous example can be written as:
...
in the first queue that it acquires. \textstyleCodeStyleChar{trylock()}
is an alternative function for \textstyleCodeStyleChar{lock()}. The
only difference is that \textstyleCodeStyleChar{trylock()} is
non-blocking. If the thread cannot acquire the mutex immediately, the
function fails. The most suitable way to use
\textstyleCodeStyleChar{trylock()} is to combine it with an
\textstyleCodeStyleChar{if} statement, where the
\textstyleCodeStyleChar{then} body unlocks the mutex after finishing
...
Both \textstyleCodeChar{lock()} and \textstyleCodeStyleChar{trylock()}
return a reference to the mutex or the object they acquired (upon
succeeding in case of \textstyleCodeStyleChar{trylock()}). This makes
...
\textstyleCodeStyleChar{L1} or \textstyleCodeStyleChar{L2} succeeds
...
\textstyleCodeChar{seq()} would look like this:
...
With the use of the initial clause, \textstyleCodeChar{seq()} is
self-contained and thread safe. Note the use of the local variable
\textstyleCodeChar{n} to temporarily hold the value of the counter
\textstyleCodeChar{i} while still in the critical region. That is
because once the thread leaves the critical region, there is no
guarantee that the value of \textstyleCodeChar{i} would remain the same
before returning the value of \textstyleCodeChar{i}. Using the variable
\textstyleCodeChar{n} guarantees that the value returned is correct
even if the value of \textstyleCodeChar{i} is changed by another
thread.
...
The \textstyleCodeChar{produce()} and \textstyleCodeChar{consume()}
...
structure is shared, by passing it to the \textstyleCodeChar{mutex()}
function. The \textstyleCodeChar{mutex()} function takes a second
...
example, the list \textstyleCodeChar{L} and the table
\textstyleCodeChar{T} in the following example share the same mutex:
...
In all cases, \textstyleCodeChar{lock(L)} and
\textstyleCodeChar{lock(T)} lock the same mutex, serializing execution
...
\textstyleCodeChar{L}\textstyleCodeChar{[1]} has the value of 3 and two
threads are trying to increment \textstyleCodeChar{L[1]}:

L[1] := L[1] + 1

the result in \textstyleCodeChar{L[1]} could be 4 or 5. That is because
reading \textstyleCodeChar{L[1]} (the right side of the assignment) and
storing the result back in \textstyleCodeChar{L[1]} are two different
...
mutex. If \textstyleCodeChar{L} is marked as shared (passed to the
\textstyleCodeChar{mutex()} function ) it can be passed to
\textstyleCodeChar{lock()}/\textstyleCodeChar{unlock()} functions. It
...
\textstyleCodeChar{condvar()}. The return object is a condition
variable that can be used with \textstyleCodeChar{wait()} and
\textstyleCodeChar{signal()} functions. \textstyleCodeChar{wait(cv)}
blocks the current thread on the condition variable
\textstyleCodeStyleChar{cv}. The thread remains blocked until another
thread does a \textstyleCodeChar{signal(cv)}, which wakes up one thread
blocked on \textstyleCodeStyleChar{cv}. A very important aspect of
using a condition variable is that the condition variable must always
be associated with a mutex. More specifically, the
\textstyleCodeChar{wait()} function has to be always protected by a
...
\textstyleCodeChar{lock()}/\textstyleCodeChar{unlock()} functions or
the critical clause. It is important to realize that not only
\textstyleCodeChar{wait()} has to be protected by a critical region,
but also the condition or the test that leads a thread to wait on a
condition variable. \ See the following example:

if x=0 then wait(cv)

A thread wants to wait on \textstyleCodeChar{cv} if
\textstyleCodeChar{x=0}, but what happens if the value of
\textstyleCodeChar{x} has changed between the test and the call to
\textstyleCodeChar{wait(cv)}? If a second thread changes the value of
\textstyleCodeChar{x} and signals \textstyleCodeChar{cv} \ to wake up
the first thread, while the first thread transitions from the test to
\textstyleCodeChar{wait()}\textstyleCodeChar{,} it ends up missing the
...
\textstyleCodeStyleChar{wait()} with a condition variable becomes:
...
some threads are waiting on it, the \textstyleCodeChar{wait()} function
...
when \textstyleCodeChar{wait()} returns. It is a good practice to do
...
that ensures that a thread does not leave \textstyleCodeChar{wait()}
before guaranteeing the test is in a specific state as follows:

\ critical cv: while x=0 do wait(cv)

Using a \textstyleCodeChar{while} in place of \textstyleCodeChar{if}
...
\textstyleCodeChar{condvar()} allows an optional argument that refers
...
\textstyleCodeChar{condvar()} function in the original example, a mutex
can be first created and then passed along with the list to the
\textstyleCodeChar{mutex()} function. The same mutex then can be passed
to \textstyleCodeChar{condvar()}. The function binds the mutex already
...
In previous examples, calls to \textstyleCodeChar{signal()} are not
protected by any mutex. \textstyleCodeChar{signal()} does not need
...
\textstyleCodeChar{signal()} is not a problem either. Depending on the
...
The \textstyleCodeStyleChar{signal()} function takes a second optional
...
\textstyleCodeStyleChar{cv} needs to be woken up, a special
\textstyleCodeStyleChar{0} ( or \textstyleCodeStyleChar{CV\_BROADCAST})
value can be passed to \textstyleCodeStyleChar{signal()} causing it to
broadcast a wakeup call for all threads waiting on
\textstyleCodeStyleChar{cv}:

signal(cv, 0) 

or

signal(cv, CV\_\textstyleCodeStyleChar{ BROADCAST}) 
...
using the \textstyleCodeChar{@} operator. All co-expression
...
Unicon. A co-expression \textstyleCodeChar{C1} can activate another
co-expression \textstyleCodeChar{C2} using the following syntax:

\ x@C2

\textstyleCodeChar{x} is an optional value to be transmitted from
\textstyleCodeChar{C1} to \textstyleCodeChar{C2}.
\textstyleCodeChar{C1} waits until it gets activated by
\textstyleCodeChar{C2} or any other co-expression directly or
indirectly activated by \textstyleCodeChar{C2}. As mentioned earlier,
...
The \textstyleCodeChar{@} operator is not suitable for this kind of
(asynchronous) communication. Unicon adds four operators dedicated for
asynchronous communication. These are
\textstyleCodeChar{@{\textgreater}},
\textstyleCodeChar{@{\textgreater}{\textgreater}},
\textstyleCodeChar{{\textless}@} and
\textstyleCodeChar{{\textless}{\textless}@}. The operators correspond
...
\includegraphics[width=5.989in,height=1.4807in]{UTR14-img/UTR14-img1.png}
...
The first two operators are \textstyleCodeChar{@{\textgreater}} (send)
and \textstyleCodeChar{{\textless}@} (receive). These operators
...
\textstyleCodeChar{x} can be any data type including null which is
equivalent to omitting it. \textstyleCodeChar{T} refers to a thread to
which \textstyleCodeChar{x} is transmitted. \textstyleCodeChar{X} is
placed in \textstyleCodeChar{T}{\textquoteright}s inbox.
\textstyleCodeChar{X} can be picked by \textstyleCodeChar{T} using the
receive operator which is presented later. The send operator can also
have no destination such as:

x @{\textgreater}

In this case \textstyleCodeChar{x} is sent to no one, instead it is
placed in the sender{\textquoteright}s outbox. The operator can be read
as {\textquotedblleft}produce
\textstyleCodeChar{x{\textquotedblright}}. \textstyleCodeChar{x} can
...
\textstyleCodeChar{T}{\textquoteright}s outbox, a value produced by
\textstyleCodeChar{T}. Going back to the prime number example mentioned
above, \textstyleCodeChar{{\textless}@T} would be the way to pick a
prime number produced by the prime number generator thread
\textstyleCodeChar{T}. \textstyleCodeChar{{\textless}@} on the other
...
Both \textstyleCodeStyleChar{@{\textgreater}} and
\textstyleCodeStyleChar{{\textless}@ }can succeed or fail. In the case
of \textstyleCodeStyleChar{{\textless}@} the operator succeeds and
...
\textstyleCodeStyleChar{@{\textgreater}}, if the value is placed in the
...
sizes do not explode quickly by default and also to have an implicit
...
\textstyleCodeChar{{\textless}@} has an explicit source like A in
...
the \textstyleCodeStyleChar{Attrib()} Function }
...
\textstyleCodeStyleChar{Attrib()}. \textstyleCodeStyleChar{Attrib()}
uses the form \textstyleCodeStyleChar{Attrib(handle, attribcode, value,
attribcode, value, ...)}.~The integer codes used by the
\textstyleCodeStyleChar{Attrib()} function are supplied by an include
file (\textstyleCodeStyleChar{thread.icn}) with
\textstyleCodeStyleChar{\$define} symbols for the attributes. This
header file is part of a new thread package that is part of the Unicon
distribution called package \textstyleCodeStyleChar{threads}. It can be
imported to a program via:

import threads

When values are omitted, \textstyleCodeStyleChar{Attrib()} generally
...
operator \textstyleCodeStyleChar{@{\textgreater}{\textgreater}} and the
blocking receive operator
\textstyleCodeStyleChar{{\textless}{\textless}@.} These operators can
be used in the same way \textstyleCodeStyleChar{@{\textgreater}} and
\textstyleCodeStyleChar{{\textless}@} are used, except that instead of
...
Using an \textstyleCodeChar{if} statement with
\textstyleCodeStyleChar{{\textless}@} is necessary to check if the
...
\textstyleCodeStyleChar{if} can be dropped, and also the counter is not
necessary anymore. The counter is necessary in the previous case
because the loop needs to keep track of how many
\textstyleCodeStyleChar{{\textless}@} succeeded to count to 10. The
consumer can be rewritten as:

procedure consume(p)

\ \ \ every !10 do write({\textless}{\textless}@p)\ \ \# get exactly 10
values from p, block if necessary
...
\textstyleCodeStyleChar{@{\textgreater}{\textgreater}}). The receiver
...
the queue (\textstyleCodeStyleChar{{\textless}{\textless}@}). For some
...
the \textstyleCodeChar{channel()} function. 
...
and can be created using the function \textstyleCodeChar{channel()}.
\textstyleCodeChar{Channel()} is part of the
\textstyleCodeStyleChar{threads} package and takes one parameter, which
is the thread with which the connection will be initiated. If
\textstyleCodeChar{channel()} succeeds, it returns a list representing
...
each thread calls the function \textstyleCodeChar{channel()} with the
...
\ \ \ every !10@{\textgreater}c \ \ \# place values in channel c
...
its performance are described in more detail in [3]. There are major
...
\section[[1{]} Gregory Andrews and Fred Schneider. Concepts and
Notations for Concurrent Programming. ACM Computing Surveys. 15:1,
March 1983, pp 3{}-43.]{[1] Gregory Andrews and Fred Schneider.
\textit{Concepts and Notations for Concurrent Programming}. ACM
Computing Surveys. 15:1, March 1983, pp 3-43.}
\section[[2{]} David R. Butenhof. Programming with Posix Threads.
Addison Wesley, 1997.]{[2] David R. Butenhof. \textit{Programming with
Posix Threads}. Addison Wesley, 1997.}
\section[[3{]} Jafar Al Gharaibeh. Programming Language Support for
Virtual Environments. Ph.D. Dissertation, University of Idaho,
2012.]{[3] Jafar Al Gharaibeh. \textit{Programming Language Support for
