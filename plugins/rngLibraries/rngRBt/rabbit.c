 ...
 *--------------------------------------------------------------------------------
 *
 * The original version of this code was released into the public domain in
 * October 2008 (despite the copyright notice below, which has not been altered).
...
/* Routine added to advance the generator without returning any bits */
void skip_blocks(ECRYPT_ctx* ctx, int blocks)
{
  while (0 <= --blocks) {RABBIT_next_state(&(ctx->work_ctx));}
}
