...
\section{Raw IP Examples: uping and uigmp}
\index{uping}\index{uigmp}\index{raw socket}

Chapter~5 describes mode \texttt{"nr"} and the \texttt{PacketSpec}
library in package \texttt{net}. Two small programs in
\texttt{uni/progs} show the same ideas end to end. They need
privileges suitable for \texttt{SOCK\_RAW}:
...
\texttt{uping} and \texttt{uigmp} behave like small network tools by
default (\texttt{Ping}/\texttt{Igmp} session APIs). Pass
\texttt{-v} or \texttt{-d} to also exercise the lower-level
\texttt{open(..., "nr", ...)} plus PacketSpec path and print
\texttt{describe()} output. They need raw-socket privileges and are
teaching clients, not replacements for system ping(8) or tcpdump(1).

\section{Network Namespace Example: unettopo}
\index{unettopo}\index{network namespace}

Chapter~5 describes mode \texttt{"j"} and \texttt{package net} link
helpers. \texttt{uni/progs/unettopo.icn} builds a two-node veth pair
in isolated Linux network namespaces and pings across it:
...
The program needs the \texttt{Network namespaces} feature and either
\texttt{CAP\_NET\_ADMIN} or a working unprivileged user-namespace
bootstrap. It is a topology demo, not a general network simulator.
