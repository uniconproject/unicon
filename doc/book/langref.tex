...
\noindent
\index{channel}\textsf{channel(TH)} creates a communications channel
...
\noindent
\index{lock}\textsf{lock(x)} locks the mutex {\textit x} or the mutex
...
\noindent
\index{mutex()}\textsf{mutex()} creates a new mutex. For \textsf{mutex(x)}
...
\noindent
\index{signal}\textsf{signal(x, y)} signals the condition variable
...
\noindent
\index{spawn()}\textsf{spawn(ce)} launches co-expression {\textit ce} as an
...
\noindent
\index{lock}\textsf{trylock(x)} locks the mutex {\textit x} or the mutex
...
\noindent
\index{lock}\textsf{unlock(x)} unlocks the mutex {\textit x} or the mutex
associated with thread-safe object {\textit x}.
...
\noindent
\index{wait()}\textsf{wait(x)} waits for {\textit x}. If {\textit x} is
