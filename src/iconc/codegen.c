   ...
   int reachable;
{
   ...
   fprintf(codefile, "{\n");
   fprintf(codefile, "   struct PF%s_%s r_f;\n", prefix, name);
   ...
   if (!optim || ret_flag & (DoesRet | DoesSusp))
      fprintf(codefile, "   r_f.rslt = r_rslt;\n");
   else
      fprintf(codefile, "   r_f.rslt = NULL;\n");
   ...
   if (optim) {
      ...
      if (cur_proc->nargs < 0) {
         ...
         fprintf(codefile,"       varargs(NULL, 0, &r_f.t.d[%d]);\n",
            first_arg + nparms);
         fprintf(codefile,"   else\n");
         fprintf(codefile,
            "       varargs(&r_args[%d], %s, &r_f.t.d[%d]);\n", nparms,
      ...
      if (nparms > 0) {
...
#ifdef OptimizeLoop
         if (nparms == 2)  {
            fprintf(codefile, "   if (r_nargs > 0) {\n");
            fprintf(codefile, "      deref(&r_args[0], &r_f.t.d[0]);\n");
            fprintf(codefile, "      if (r_nargs > 1)\n");
            fprintf(codefile, "         deref(&r_args[1], &r_f.t.d[1]);\n");
            fprintf(codefile, "      else\n");
 	    ...
 	    fprintf(codefile, "      r_f.t.d[1].dword = D_Null;\n");
            ...
            if (first_arg != 0)
               fprintf(codefile, "      deref(&r_args[i], &r_f.t.d[i + %d]);\n",  first_arg);
            else
               fprintf(codefile, "      deref(&r_args[i], &r_f.t.d[i]);\n");
            fprintf(codefile, "   for( ; i < %d ; ++i)\n", nparms);
            if (first_arg != 0)
   ...
   fprintf(codefile, "   r_f.t.num = %d;\n", ntend);
   fprintf(codefile, "   r_f.t.previous = tend;\n");
   fprintf(codefile, "   tend = (struct tend_desc *)&r_f.t;\n");
   if (line_info) {
      fprintf(codefile, "   r_f.debug.old_line = line_num;\n");
      fprintf(codefile, "   r_f.debug.old_fname = file_name;\n");
      }
   if (debug_info) {
      fprintf(codefile, "   r_f.debug.proc = (struct b_proc *)&BP%s_%s;\n",
         prefix, name);
   ...
   for ( ; cd != NULL; cd = cd->next) {
      switch (cd->cd_id) {
         ...
         case C_PFail:
            ...
            fprintf(codefile, "   tend = r_f.t.previous;\n");
            fprintf(codefile, "   pfp = r_f.old_pfp;\n");
            fprintf(codefile, "   glbl_argp = r_f.old_argp;\n");
            if (line_info) {
               fprintf(codefile, "   line_num = r_f.debug.old_line;\n");
               fprintf(codefile, "   file_name = r_f.debug.old_fname;\n");
               }
            fprintf(codefile, "   return A_Resume;\n");
            break;
...
int outer;
   ...
   switch (var->flag) {
      ...
      case F_Dynamic:
         frame(outer);
         fprintf(codefile, ".t.d[%d]", var->val.index);
         break;
      case F_Argument:
...
int outer;
   ...
   switch (loc->loc_type) {
      ...
      case V_Temp:
          ...
          */
         frame(outer);
         fprintf(codefile, ".t.d[%d]", cur_proc->tnd_loc + loc->u.tmp);
         break;
      case V_ITemp:
         /*
