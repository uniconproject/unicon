...
\>local wchar, line \\
\\
\>wchar := \&letters ++ \&digits ++ '{\textbackslash}'-' \\
\>while line := read() do \\
...
\iconcode{
static wchar \\
initial wchar := \&letters ++ \&digits ++ '{\textbackslash}'-'
}
...
\>return (s {\textbar}{\textbar} " ") ? (bal() = *s+1) \\
end
}
...
\iconcode{
procedure main(args) \\
\>(*args = 1) {\textbar} stop("Need a file!") \\
\>f := open(args[1]) {\textbar} stop("Couldn't open ", args[1]) \\
\>wordlist := table() \\
\>lineno := 0 \\
\ \\
...
\>L := sort(wordlist) \\
\>every l := !L do \{ \\
\>\>writes(l[1], "{\textbackslash}t") \\
\>\>linelist := "" \\
\>\>\# Collect line numbers into a string \\
\>\>every linelist {\textbar}{\textbar}:= (!l[2] {\textbar}{\textbar} ", ") \\
\>\>\# trim the final ", " \\
\>\>write(linelist[1:-2]) \\
\>\>\} \\
end \\
...
\>\>\>result {\textbar}{\textbar}:= tab(j) {\textbar}{\textbar} repl \\
\>\>\>tab(ReMatch(re)) \\
\>\>\>\} \\
...
\iconcode{
igrep mypaper.txt "read{\textbar}write" "IO Operation"
}
...
\\
procedure factor(s) \\
\>s ? suspend ="a" {\textbar} ="b" {\textbar} ="c" {\textbar}  ( ="("
{\textbar}{\textbar} expr(tab(bal(')'))) {\textbar}{\textbar} =")" ) \\
end
}
