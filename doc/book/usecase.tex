\clearpage\section{Chapter 12: Use Cases, Statecharts, and Collaboration
Diagrams}
...
The \index{association}\textit{association}\textit{s} between use cases
...
\index{elaboration}\textit{elaboration}\textit{s} between use cases are
...
statechart}\textit{event}. In Figure 12-3 the events were things like
...
If you have a condition on a transition, it might make sense for that
...
\index{instance!class}instances of classes, and lines depict the
...
and numbered to show both the sequence and the \index{tree}tree
structure of the nested messages. In the general case more than one
message number can be annotated for a given \index{link!association
instance}link, since multiple messages may be transmitted between the
...
In terms of primacy and chronological order, for most applications you
