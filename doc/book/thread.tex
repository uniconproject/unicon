...
\\
\ \ \ thr := serial() \ \ \ \  \# serial() returns the thread serial number\\
\ \ \ critical mtx: tls := if not member(TT,thr) then TT[thr] := table() else TT[thr]\\
\ \ \ return tls[var]\\
end
}
\noindent
In fact, the actual implementation of \texttt{TLS} is
\iconcode{
procedure TLS(var)\\
...
\noindent
which takes full advantage of the automatic locking and unlocking features discussed earlier.
