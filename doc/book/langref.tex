...
elements consisting of text lines. Cryptographic handles and SSH
sessions with no channel are not line streams; \texttt{!x} fails.
...
Cryptographic handles, SSH files, TLS sockets, network sockets, and
network-namespace handles may
be subscripted by a status field name for a non-destructive peek
(\texttt{h["type"]}, \texttt{c["exitstatus"]}, \texttt{f["ttl"]},
\texttt{conn["cipher"]}, \texttt{ns["name"]}); an unknown name is a runtime error
(1302 crypto, 1326 TLS, 1331 SSH, 1310 sockets, 1336 netns) and an
unpopulated field fails. Boolean peeks succeed with \texttt{"yes"} or
\texttt{\&null}. \texttt{f["*"]} is a table snapshot of the answerable
fields. Peeking a closed handle is error 174.
\texttt{key(f)} that has not yet produced a name also raises 174 if the
handle is already closed; after the first suspend, \texttt{close()}
makes the generator fail instead of raising.
\texttt{f["groups"]}, \texttt{s["authmethods"]}, \texttt{h["san"]}, and
\texttt{conn["certchain"]} produce a list (a single item is still a
list of length 1); other peeks are a string, integer, or \texttt{\&null}.
\texttt{key(f)} generates the answerable field names, matching
\texttt{key()} on a messaging connection.
...
\index{Attrib}
\noindent
{\bf Attrib(x, ...) : x* } \hfill {\bf set attributes}

\noindent
\texttt{Attrib()} sets attributes of its first argument.
For a network socket file, trailing \texttt{"\textit{name}=\textit{value}"}
strings set options (including \texttt{join}/\texttt{leave} for multicast
memberships), using the same names as \texttt{open()} socket attributes
and \texttt{WAttrib()}. Status is peeked with \texttt{f["\textit{name}"]},
not with a bare name to \texttt{Attrib()}.
Boolean values are \texttt{yes}/\texttt{no}; numeric values are
integers. Socket attributes do not require the Concurrent build
feature.
SSH status (\texttt{exitstatus}, \texttt{stderr}, \texttt{eof}, and
session fields such as \texttt{fingerprint}) is peeked with
\texttt{c["\textit{name}"]}; \texttt{Attrib()} has no SSH setters.
Network-namespace status (\texttt{name}, \texttt{persist},
\texttt{userns}, \texttt{refcount}) is peeked with
\texttt{ns["\textit{name}"]}; \texttt{Attrib()} has no netns setters.
For a cryptographic handle, assignments such as \texttt{op=}, \texttt{alg=},
\texttt{cipher=}, \texttt{iv=}, and \texttt{sig=} are allowed only in
the idle window between operations. Status including \texttt{type}
(material role), \texttt{op} (operation), certificate identity, and a
non-destructive hash \texttt{hash} peek uses \texttt{h["\textit{name}"]}.
...
\texttt{key(M)} generates header names on a messaging connection
\texttt{M} (including \texttt{Status-Code} and \texttt{Reason-Phrase}).
\texttt{key(f)} generates answerable status-field names on a cryptographic
handle, SSH file, TLS socket, network socket, or network-namespace
handle; \texttt{f[k]} peeks
...
graphics window\\
 \ \ \ \ \texttt{"j"\ \ \ \ }create a
Linux network namespace handle\\
...
encrypt the network socket; alone or as
\texttt{"er"}/\texttt{"eh"}, also open cryptographic
handles (hash/HMAC/sign/encrypt; see Chapter~6).
With a file read/write mode, \texttt{"re"}~/~\texttt{"we"} open a
file-transform crypto handle. With UDP
(\texttt{"nue"}~/~\texttt{"naue"}), encrypt means DTLS.\\
...
\texttt{rcvbuf}, \texttt{sndbuf}, \texttt{proto}, \texttt{hdrincl});
boolean values are \texttt{yes} or \texttt{no}. \texttt{iface}
accepts an IPv4 address, a numeric interface index, or (on UNIX) an
interface name; when omitted, a multicast join covers every UP IPv4
interface and a multicast send uses the first non-loopback IPv4
address (else loopback). \texttt{ttl} sets unicast and multicast hop
limits; \texttt{mcastloop} controls delivery of the host's own
multicasts (default on); \texttt{rcvbuf}/\texttt{sndbuf} take a byte
size. Listeners default to \texttt{reuseaddr=yes}; multicast binds
also get \texttt{reuseport=yes} where available. Binding a UDP socket
to a multicast group joins that group; \texttt{source@group:port} or
\texttt{source=} performs a source-specific join instead (IPv4 and
IPv6; for IPv6 SSM prefer an explicit \texttt{iface=}). Sending to
\texttt{255.255.255.255} enables broadcast.
Mode \texttt{"nr"} opens a raw IP socket and requires
\texttt{proto=\textit{name{\textbar}n}} (for example \texttt{icmp},
\texttt{igmp}, or 0--255); the host argument is a bare address with
no port. Optional \texttt{hdrincl=yes} sets \texttt{IP\_HDRINCL}.
Packet bytes are built by the application (see package \texttt{net}
/ \texttt{PacketSpec} in Chapter~5). The same attributes can be
changed after \texttt{open()} with \texttt{Attrib()} (except
\texttt{proto}, which is fixed at \texttt{socket()} time); status is
peeked with \texttt{f["\textit{name}"]} (see
Chapter~5 for examples).
Mode \texttt{"j"} (Linux, feature \texttt{Network namespaces})
creates a network namespace named \texttt{s1}; trailing attributes may
include \texttt{persist=yes}, \texttt{bridge=yes}, and
\texttt{userns=yes}. Status is peeked with \texttt{ns["name"]},
\texttt{ns["persist"]}, \texttt{ns["userns"]}, and
\texttt{ns["refcount"]}; \texttt{key(ns)} generates those names.
See Chapter~5.
...
\texttt{read()} or \texttt{reads()} cannot be performed on a plain UDP
socket; use receive. Encrypted UDP (DTLS, modes \texttt{"nue"}~/~\texttt{"naue"})
uses \texttt{write}/\texttt{read} through the SSL layer like TLS.
UDP sockets must be in the INET domain; the
...
\texttt{password=}, \texttt{hostkeyfile=}, \texttt{cmd=} (requires mode
\texttt{"hc"}),
\texttt{term=}/\texttt{cols=}/\texttt{rows=} (interactive PTY), and
\texttt{channel=} (\texttt{yes}/\texttt{no}; \texttt{channel=no} is a
transport-only session), and \texttt{verifyPeer=} (\texttt{yes}/\texttt{no}).
Mode \texttt{"h-"} or \texttt{verifyPeer=no} skips host-key verification.
\texttt{open(s, "hc", ...)} on an existing SSH file opens another channel on
the same session; \texttt{open(s, "hs", "path=\textit{path}", ...)}
opens a remote file or directory over SFTP. See Chapter~6.

When opening a cryptographic handle (modes \texttt{"e"}, \texttt{"er"},
or \texttt{"eh"}, without a network \texttt{"n"}), the first argument is
a digest name (\texttt{"eh"}), a key/certificate path, a raw key string
(\texttt{"er"}), or another crypto handle that supplies material.
Trailing attributes include \texttt{op=} (\texttt{hash}, \texttt{hmac},
\texttt{sign}, \texttt{verify}, \texttt{encrypt}, \texttt{decrypt}),
\texttt{alg=}, \texttt{cipher=}, \texttt{iv=}, \texttt{sig=}, and
\texttt{keypass=} (key-file passphrase; \texttt{password=} is an alias).
Defaults are \texttt{sha256} and \texttt{aes-256-gcm}.
Modes \texttt{"re"} and \texttt{"we"} take a data-file path as the
target and obtain key material from \texttt{key=} or a handle attribute.
Encrypted sockets (\texttt{"ne"}, \texttt{"nue"}, \ldots) also accept
crypto material handles as attributes so identity can be reused across
connections. Mode \texttt{"ne-"} or \texttt{verifyPeer=no} skips peer
certificate verification; \texttt{keypass=} unlocks an encrypted
private key. See Chapter~6.
...
returned value is a record of type
\texttt{posix\_message(addr, msg, saddr, daddr, ttl, proto)},
containing the sender address (\texttt{host:port}), the message
bytes, and IP-style \texttt{saddr}/\texttt{daddr}.
On UDP, \texttt{saddr} is the numeric peer and \texttt{daddr} is
null. On an IPv4 raw datagram that includes a header,
\texttt{saddr}, \texttt{daddr}, \texttt{ttl}, and \texttt{proto}
come from that header.
For an SSH channel file, \texttt{receive(c)} instead
...
co-expression. \texttt{ce} must be a co-expression; a procedure is
not launched. An optional leading network-namespace file
(\texttt{spawn(ns, ce, ...)}) joins that namespace on the new OS
thread before any Icon code runs. The three optional integers specify
the memory in bytes allocated for the thread's block and string
regions and stack size. The defaults are 10\% of the corresponding
sizes for the main thread.
...
execution of a program in a separate process. An optional leading
network-namespace file (\texttt{system(ns, x, ...)}) runs the child
inside that namespace. \texttt{x} can be either
