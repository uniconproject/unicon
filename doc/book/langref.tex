...
Large integer literals in a program are converted to actual
...
can avoided without major effort, it is probably worth doing so.}\medskip
...
\medskip\noindent{\sffamily\bfseries \bf Note:}\ {\sffamily
The integer must be a decimal integer (including the optional
+ or - sign); Radix integer literals are not supported in an exponent.}\medskip
...
\noindent
{\bf number1 \~{}= number2 }\index{equal!numeric =}  \hfill {\bf not equal}
...
\noindent
{\bf string1 == string2 }\index{string!comparison}\index{equal!string ==} \hfill {\bf string equal}
...
\noindent
{\bf x1 === x2 }\index{equal!reference ===} \hfill {\bf equivalence}
...
\noindent
{\bf var1 :=: var2 }\index{swap} \hfill {\bf swap}\WarningNotThreadSafe


\noindent
{\bf var {\textless}- expr }\index{assignment!reversible}\index{reversible assignment} \hfill {\bf reversible assignment}


\noindent
{\bf var1 {\textless}-{\textgreater} var2 }\index{reversible swap} \hfill {\bf reversible swap}\WarningNotThreadSafe
...
\bigskip\hrule\vspace{0.1cm}
\noindent
{\bf [x] @ co-expression }\index{activate co-expression} \hfill {\bf activate co-expression}
...
\bigskip\hrule\vspace{0.1cm}
\noindent
{\bf [x] @$>$ [y] }\index{activate co-expression} \hfill {\bf send message}
 \linebreak
\noindent
...
\bigskip\hrule\vspace{0.1cm}
\noindent
{\bf [x] $<$@ [y] }\index{activate co-expression} \hfill {\bf receive message}
 \linebreak
\noindent
...
\bigskip\hrule\vspace{0.1cm}
\noindent
{\bf ( expr [, expr]* ) }\index{mutual evaluation} \hfill {\bf mutual evaluation}


\noindent
{\bf p ( expr [, expr]* ) }\index{invocation} \hfill {\bf invocation}
...
\noindent
{\bf \textbf{[} \textbf{]} }\index{empty list} \hfill {\bf empty list creation}


\noindent
{\bf \textbf{[} expr [, expr]* \textbf{]} }\index{list creation} \hfill {\bf list creation}


\noindent
{\bf \textbf{[:} expr \textbf{:]} }\index{list creation} \hfill {\bf list comprehension}


\noindent
{\bf \textbf{[} expr : expr [; expr : expr ]* \textbf{]} }\index{table!initializer} \hfill {\bf
initialized table creation}


\noindent
{\bf expr1 \textbf{[} expr2 [, expr]* \textbf{]} }\index{subscript} \hfill {\bf subscript}


\noindent
{\bf expr1 \textbf{[} expr2 : expr3 \textbf{]} }\index{slice}\index{subsection} \hfill {\bf subsection}\WarningNotThreadSafe
...
\bigskip\hrule\vspace{0.1cm}
\noindent
{\bf \{ expr [; expr]* \} }\index{compound expression} \hfill {\bf compound expression}


\noindent
{\bf p \{ expr [, expr]* \} }\index{programmer defined control structure}\index{control structure!programmer defined} \hfill {\bf programmer defined control structure}
...
sets the access permissions ("mode") of a string filename (or on UNIX systems,
an open file) \texttt{f} to a string or integer mode \texttt{m}. The mode
indicates the change to be performed. The string is of the form
...
\begin{xtabular}{m{3.00cm}m{10.00cm}}
\ \ \ \ \ \ \ \texttt{r} & read\\
\ \ \ \ \ \ \ \texttt{w} & write\\
\ \ \ \ \ \ \ \texttt{x} & execute\\
\ \ \ \ \ \ \ \texttt{R} & read if any other set already has r\\
\ \ \ \ \ \ \ \texttt{W} & write if any other set already has w\\
\ \ \ \ \ \ \ \texttt{X} & execute if any other set already has x\\
\ \ \ \ \ \ \ \texttt{s} & setuid (if the first part contains u and/or setgid if the first part contains g\\
\ \ \ \ \ \ \ \texttt{t} & sticky if the first part has o\\
...
\texttt{statements}, \texttt{dsn}\\ \noindent that describes the details of
...
with fields:\\ \texttt{columnname}, \texttt{sequencenumber}\\
\noindent containing information about the primary keys in \texttt{tablename}.
...
\texttt{maxcharlitlen}, \texttt{maxcolnamelen}, \texttt{maxgroupbycols},\\ \texttt{maxorderbycols},
\texttt{maxindexcols}, \texttt{maxselectcols}, \texttt{maxtblcols},\\ \texttt{maxcursnamelen}, \texttt{maxindexsize},
\texttt{maxrownamelen}, \texttt{maxprocnamelen},\\ \texttt{maxqualnamelen}, \texttt{maxrowsize},
\texttt{maxrowsizelong}, \texttt{maxstmtlen},\\ \texttt{maxtblnamelen}, \texttt{maxselecttbls},
\texttt{maxusernamelen}\\
\noindent that contains the upper bounds of the database
for many parameters.
...
\noindent
\index{delay(i)}\texttt{delay(i)} pauses the program for at least
...
\begin{xtabular}{m{2.00cm}m{5.00cm}}
\ \ \texttt{s} & shared lock\\
\ \ \texttt{x} & exclusive lock\\
\ \ \texttt{b} & don't block when locking\\
\ \ \texttt{u} & unlock\\
\end{xtabular}
\end{flushleft}
\noindent
...
\texttt{{\textquotedblleft}gfp{\textquotedblright}}.\\
\noindent Used by monitors.
...
\index{load!Unicon program}\index{load()}loads the icode file named \texttt{s} and returns that
...
\noindent
\texttt{mkdir(path,mode)}\index{directory!create}\index{create!directory}\index{mkdir()}
creates a new directory named \texttt{path} with
...
\begin{xtabular}{m{2.00cm}m{10.00cm}}
\ \ \texttt{"a"} & append; write after current contents\\
\ \ \texttt{"b"} & open for both reading and writing (b does not mean binary mode!)\\
\ \ \texttt{"c"} & create a new file and open it\\
\ \ \texttt{"d"} & open a [NG]\index{DBM}DBM database\\
\ \ \texttt{"g"} & create a 2D graphics window\\
\ \ \texttt{"gl"} & create a 3D graphics window\\
\ \ \texttt{"n"} & connect to a remote TCP network socket\\
\ \ \texttt{"na"} & accept a connection from a TCP network socket\\
\ \ \texttt{"nau"} & accept a connection from a UDP network socket\\
\ \ \texttt{"nl"} & listen on a TCP network socket\\
\ \ \texttt{"nu"} & connect to a UDP network socket\\
\ \ \texttt{"m"} & connect to a messaging server (HTTP, HTTPS, SMTP, POP, ...)\\
\ \ \texttt{"o"} & open an \index{ODBC}ODBC connection to a (typically \index{SQL}SQL) database\\
\ \ \texttt{"p"} & execute a program given by command line s1 and open a pipe to it\\
\ \ \texttt{"r"} & read\\
\ \ \texttt{"t"} & use text mode, with newlines translated\\
\ \ \texttt{"u"} & use a binary untranslated mode\\
\ \ \texttt{"w"} & write\\
\end{xtabular}
\end{flushleft}
\medskip\noindent
...
for subtraction, while \texttt{proc("-")} produces the
...
\noindent Many of these fields are POSIX
...
\medskip\noindent
{\bf Caveat:} This is not supported by the optimizing
...
\texttt{\&control}, \texttt{\&shift}, \texttt{\&meta},\\
\noindent and if 3D
attribute \texttt{{\textquotedblleft}pick=on{\textquotedblright}}:\\
\texttt{\&pick}.\\
\noindent\texttt{Event()} fails if the timeout expires before
an event occurs.
...
\texttt{width,pallet,data}\\
\noindent or\\
\texttt{width,\#,data},\\
\noindent where pallet is a
...
replaced by the \textit{text} within the current file.

\medskip\noindent{\sffamily\bfseries \bf Note:}\ {\sffamily
\$define does not support arguments, unlike C.}\medskip
...
\linebreak
\index{ifdef symbol}{\bf \$ifndef \textit{symbol} } \hfill {\bf conditional compilation}
\linebreak
{\bf \$else } \hfill {\bf conditional alternative}
\linebreak
{\bf \$endif } \hfill {\bf end of conditional code}

The subsequent lines of code, up to an \texttt{\$else} or \texttt{\$endif}, are discarded
unless \textit{symbol} is defined by some \texttt{\$define} directive. \texttt{\$ifndef}
reverses this logic.
...
\bigskip\hrule\vspace{0.1cm}
\noindent
{\bf \$line \textit{number} [\textit{filename}] } \hfill {\bf source code line}
\index{source code \$line}\linebreak
{\bf \#line \textit{number} [\textit{filename}] } \hfill {\bf source code line}
...
\begin{flushleft}
\tablehead{
    \bfseries Preprocessor Symbol &
    \bfseries Feature \\\hline\\
}
\begin{xtabular}{m{6.00cm}m{6.00cm}}
\ \ \ \ \texttt{\_V9} & Version 9\\
\ \ \ \ \texttt{\_CMS} & CMS\\
\ \ \ \ \texttt{\_MACINTOSH} & Macintosh\\
\ \ \ \ \texttt{\_MSDOS\_386} & MS-DOS/386\\
\ \ \ \ \texttt{\_MS\_WINDOWS\_NT} & MS Windows NT\\
\ \ \ \ \texttt{\_MSDOS} & MS-DOS\\
\ \ \ \ \texttt{\_MVS} & MVS\\
\ \ \ \ \texttt{\_OS2} & OS/2\\
\ \ \ \ \texttt{\_PORT} & PORT\\
\ \ \ \ \texttt{\_UNIX} & UNIX\\
\ \ \ \ \texttt{\_POSIX} & POSIX\\
\ \ \ \ \texttt{\_DBM} & \index{DBM}DBM\\
\ \ \ \ \texttt{\_VMS} & VMS\\
\ \ \ \ \texttt{\_ASCII} & \index{ASCII}ASCII\\
\ \ \ \ \texttt{\_EBCDIC} & \index{EBCDIC}EBCDIC\\
\ \ \ \ \texttt{\_CO\_EXPRESSIONS} & co-expressions\\
\ \ \ \ \texttt{\_CONSOLE\_WINDOW} & console window\\
\ \ \ \ \texttt{\_DYNAMIC\_LOADING} & dynamic loading\\
\ \ \ \ \texttt{\_EVENT\_MONITOR} & event monitoring\\
\ \ \ \ \texttt{\_EXTERNAL\_FUNCTIONS} & external functions\\
\ \ \ \ \texttt{\_KEYBOARD\_FUNCTIONS} & \index{keyboard}keyboard functions\\
\ \ \ \ \texttt{\_LARGE\_INTEGERS} & large integers\\
\ \ \ \ \texttt{\_MULTITASKING} & multiple programs\\
\ \ \ \ \texttt{\_PIPES} & pipes\\
\ \ \ \ \texttt{\_RECORD\_IO} & record I/O\\
\ \ \ \ \texttt{\_SYSTEM\_FUNCTION} & system function\\
\ \ \ \ \texttt{\_MESSAGING} & messaging\\
\ \ \ \ \texttt{\_GRAPHICS} & graphics\\
\ \ \ \ \texttt{\_X\_WINDOW\_SYSTEM} & X Windows\\
\ \ \ \ \texttt{\_MS\_WINDOWS} & MS Windows\\
\ \ \ \ \texttt{\_WIN32} & Win32\\
\ \ \ \ \texttt{\_PRESENTATION\_MGR} &  Presentation Manager\\
\ \ \ \ \texttt{\_DOS\_FUNCTIONS} &  MS-DOS extensions\\
\ \ \ \ \texttt{\_DEVMODE} &  developer mode\\
\end{xtabular}
\end{flushleft}
...
\begin{flushleft}
\tablehead{
    \bfseries Error Number &
    \bfseries Description \\\hline\\
}
\begin{xtabular}{m{2.00cm}m{13.00cm}}
...
1207  &  invalid field in header\\
1208  &  messaging file expected\\
1209  &  cannot determine smtpserver (set UNICON\_SMTPSERVER)\\
1210  &  cannot determine user return address (set UNICON\_USERADDRESS)\\
1211  &  invalid email address\\
1212  &  server error\\
1213  &  POP messaging file expected\\
\end{xtabular}
...
The Unicon syntax is described here using ``Railroad diagrams''.

\medskip\noindent{\sffamily\bfseries \bf Note:}\ {\sffamily
A newline may be substituted for the semicolon symbol (i.e.
\raisebox{-1.3mm}{\tikz \node[scale=0.75,T]{;};}
) wherever it appears in the diagrams. }\medskip

Some non-terminal symbols, which could be
