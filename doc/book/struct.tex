...
\iconcode{
\>opts := options(args, "n+") \\
\>n := {\textbackslash}opts["n"] {\textbar} 6 \\
\>if n {\textless}= 0 then stop("-n needs a positive numeric parameter")
}
...
\>\>rows[r] {\textless}- up[n+r-c] {\textless}- down[r+c-1] {\textless}- 1 do \{
}
...
\>initial \{ \\
\>\>count := 0 \\
\>\>line := repl("{\textbar} \ \ ", n) {\textbar}{\textbar} "{\textbar}" \\
\>\>border := repl("-{}-{}-{}-", n) {\textbar}{\textbar} "-" \\
\>\>\} \\
\>write("solution: ", count+:=1, "{\textbackslash}n \ ", border) \\
\>every line[4*(!solution - 1) + 3] {\textless}- "Q" do \{ \\
\>\>write(" \ ", line, {\textquotedblleft}{\textbackslash}n \ ", border) \\
\>\>\} \\
\>write() \\
end
}
