...
{ /* 
  ...
  /* ---------------------------------------------------------------------- */
  props->typeFlags = RngTypeFlag(T_Integer) | RngTypeFlag(T_Intarray);
  /* Why are there two types specified for putSeed?
   *
   * T_Intarray is used to alter both key and IV (i.e. a full setup).
   * T_Integer is used to just alter the IV after a full set up.
   *
   * This means that putSeed() must detect whether a full setup has been done.
   *
...
int putSeed(word type, word size, void *param)
{
   ...
   */
  if ((type != T_Integer) && (type != T_Intarray)) {
  ...
  if (type == T_Intarray) {
    ...
    if ((size * 8) < 128 + 64) {
      ...
      state->hiowpe = 1.0;      /* Clear any stored value */
    }
  } else { /* type == T_Integer */
    state = (ECRYPT_ctx *)runtime.getRngState(); /* get the location of generator's state */
...
double getRandomFpt(void)
{
   ...
   */
  if (state->hiowpe < 1.0) { /* There is a number waiting to go */
    double ans = state->hiowpe;
    state->hiowpe = 1.0;        /* Clear the stored value */
    return ans;
  } else {
    u64 ans[2];
    /* Get a block's worth of random bits */
    ECRYPT_keystream_bytes(state, (u8 *)ans, sizeof(ans));
    state->hiowpe = RanScale * ans[1]; /* Stash one away for later */
 ...
 * It is expected to return a bit vector of the required size, although it is
 * also allowed to fail.
 *
