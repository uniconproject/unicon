:title: iyacc: A Parser Generator for Icon and Unicon
:author: Ray Pereda and Clinton Jeffery
:trnumber: 3a
:date: 2018-02-21
:abstract: iyacc is software tool for building language processors. It is
   based on byacc, a well-known tool for the C programming language. This
   report describes how to use the tool.
:docclass: report
...
wish to also read "Lex & Yacc", by John Levine :cite:`Levine92`.
...
A Parser for a Desktop Calculator
=================================
...
generator program such as ulex :cite:`Ray03`. In the example, a very crude
...
``'+'``, ``'-'``, ``'*'``, ``'/'``, ``'^'``, ``'('``, and ``')'`` are not declared in the above
...
file, y_tab.icn, containing the ``$define``'s for the terminal symbols
