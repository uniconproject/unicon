...
expense of fully occupying one CPU in a spin-lock.

\medskip\noindent{\sffamily\bfseries \bf Note:}\ {\sffamily Declaring
...
would be invisible in the main thread.}\medskip
...
\texttt{trylock()} may fail to lock any of the lists, leaving
\texttt{item} unprocessed.}\medskip

Depending on what the code needs to do, if it is
...
\medskip\noindent{\sffamily\bfseries \bf Note:}\ {\sffamily
The use of the local variable \texttt{n} to temporarily
hold the value of the counter \texttt{i} while still in the critical
region is because once the thread leaves the critical region, there is no
...
correct, even if the value of \texttt{i} is changed by another thread.}\medskip
...
\medskip\noindent{\sffamily\bfseries \bf Note:}\ {\sffamily
Calling \texttt{ClosePool} directly in a worker thread will lead to
...
\medskip\noindent{\sffamily\bfseries \bf Note:}\ {\sffamily
This design has a race condition --- it is possible that an analysis
...
\texttt{countingThread} variable is not null.}\medskip
...
\medskip\noindent{\sffamily\bfseries \bf Note:}\ {\sffamily
Instead of using a thread from the pool as an accumulator, one is
created on the fly (this is another way of avoiding the start-up race discussed
in the previous example).}\medskip

The \texttt{du} procedure analyzes a directory, adding
...
\medskip\noindent{\sffamily\bfseries \bf Note:}\ {\sffamily
\texttt{IsIdle()} must be true before checking the message queue is
...
result would be that we'd ignore the final message, or messages).}\medskip
