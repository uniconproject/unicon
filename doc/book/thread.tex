...
In the following version, which uses the \texttt{WorkPool} library package, each
...
\\
\>\# Start a "counting thread" that accumulates the answers from the analysis threads.\\
...
\>\# It will accumulate all the answers that have previously been sent before receiving "end".\\
\>"end" @{>}{>} countingThread\\
\\
...
\>\>\>\>\>kb +:= ((st.size {<} st.blksize) | st.blksize * ceil(st.size/(0.0 + st.blksize)))/1024\\
\\
\>\>\>\>"d": \# Directory - hand it off to another worker thread to analyze in parallel\\
