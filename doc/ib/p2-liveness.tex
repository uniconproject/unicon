...
%%      rr/.style={rounded rectangle, fill=white, draw, minimum height=6mm}
%%    ]
%% % Make a simple column/row grid of labelled points
%% \begin{scope}[inner sep=0]
%%   \foreach \c in {0,...,9} {
%%     \foreach \r in {0,...,6} {
%%       \node[name=p\c\r] at (\c,\r) {};
%%       };
%%   };
%% \end{scope}

%% \node (pl) [rr] at (p54) {+};
%% \node (e1) [rr] at (p21) {expr$_1$};
%% \node (e2) [rr] at (p81) {expr$_2$};
%% \draw ($(p55) -(0,0.25)$) -- (pl) -- (e1) (pl) -- (e2);
%% \draw[dashed,->] ($(p22) -(0.5,0.5)$) -- (p44) to[in=270] ($(p45) + (0.5,0)$)
%%       node[at start, left] {failer};
%% \draw[dashed,->] ($(p55) + (0.5,0)$) -- ($(pl) + (0.5,0)$) node[at start, right] {resumer};
%% \node[xshift=0.5cm,anchor=east] at (p45) {failer};
%% \draw[dashed,->] (p84) -- (p85) node[at end,right] {gen};
%% \draw[dashed,->] ($(p64) - (0,0.5)$) -- ($(e2) + (0,0.5)$) node[at end,right] {resumer};

%% \draw[dashed,->] ($(e2.west)+(0,0.25)$)
%%    .. controls (p63) and (p43)
%%    .. ($(e1.east)+(0,0.25)$)
%%    node[at start,left] {failer~} node[at end, right] {~resumer};

%% \draw[dashed,->] ($(p92)+(1.5,-0.5)$) -- (p93) node[at start,right] {gen};
%% \draw[dashed,->] ($(p01)+(-0.5,0.5)$) -- (p13) node[at start, left] {gen};
%% \end{tikzpicture}
%% \end{center}
