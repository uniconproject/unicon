...
loop:
	...
	else if (cptr[1] == '-')
	{
		...
		unknown_rhs(-i);
            if (jflag)/*rwj*/
	      fprintf(f, "val_peek(%d)", i + n);
	    else if (iflag)
	      fprintf(f, "valstk[%d]", i + n + 1);
	    else
	      fprintf(f, "yyvsp[%d]", -i - n);
	    goto loop;
	}
    ...
    /* catch closing brace for Icon action */
	if (c == '}' && depth == 1 && iflag) {  
	   depth = 0;
	   ++cptr;
	   fprintf(f, "\nend\n\n");
...
{
    if (jflag) /*rwj*/
      write_section(jbanner);
    else if (iflag)
      write_section(ibanner);
    else
