...
There is not one \texttt{\&error} integer for each thread \WarningNotThreadSafe\
...
\texttt{\&pos}; \WarningNotThreadSafe assigning a value to it in one
...
\texttt{\&random}; \WarningNotThreadSafe setting it in one thread does
...
\texttt{\&pos}.
Each thread has its own instance of 
\texttt{\&subject}; \WarningNotThreadSafe assigning a value to it in one
thread does not affect the string scanning environment of any another thread.
...
Any subsequent invocations (of the procedure or method) \WarningNotThreadSafe
