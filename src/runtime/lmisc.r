...
operator{0,1} <@ rcv(x,y)
   ...
   if is:null(y) then inline {
...
#if NT
         DWORD tb;
	 if ((PeekNamedPipe(pt->main_read, NULL, 0, NULL, &tb, NULL) != 0)
		&& (tb>0)) {
#else
	 int tb;
	 fd_set readset;
	 struct timeval tv;
	 FD_ZERO(&readset);
	 FD_SET(pt->main_fd, &readset);
	 tv.tv_sec = tv.tv_usec = 0;
	 if (select(pt->main_fd+1, &readset, NULL, NULL, &tv) > 0) {
	    ...
	    DEC_NARTHREADS;
#if NT
            status = ReadFile(pt->main_read, sbuf, i, &tb, NULL);
#else
	    tb = read(pt->main_fd, sbuf, i);
            status = (tb != -1);
#endif
	    INC_NARTHREADS_CONTROLLED;
	    if (!status) fail;
