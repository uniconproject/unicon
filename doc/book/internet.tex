...
\>   (s ||:= ready(net)) ? \{ \\
\>\>     = {\textquotedbl}HSP{\textbackslash}n{\textquotedbl} {\textbar} \{
write(net, {\textquotedbl}wrong protocol{\textquotedbl}); fail \} \\
\>\>     game := tab(many(gamenamechars)) {\textbar} \{
                  write(net,{\textquotedbl}no game?{\textquotedbl}); fail \} \\
\>\>     = {\textquotedbl}{\textbackslash}n{\textquotedbl} \\
\>\>     owner := tab(many(gamenamechars)) {\textbar} \{
                  write(net,{\textquotedbl}no owner?{\textquotedbl}); fail \} \\
\>\>     = {\textquotedbl}{\textbackslash}n{\textquotedbl} \\
\>\>     score := tab(many(\&digits)) {\textbar} \{
                  write({\textquotedbl}no score?{\textquotedbl}); fail \} \\
\>\>     \} \\
\ \\
\>   if t[game] === \&null then \{ \\
\>\>    if not (f := open(game)) then \{ \\
\>\>\>     write(net, {\textquotedbl}No high scores here for {\textquotedbl}, game) \\
\>\>\>     fail \\
\>\>\>     \} \\
\>\>    t[game] := L := [] \\
\>\>    while put(L, read(f)) \\
...
procedure main(args) \\
\>   win\_init() \\
\>   net\_init(args[1] {\textbar} {\textquotedbl}127.0.0.1{\textquotedbl})
}
...
\>   host ?:= \{ \\
\>   \ \ \ if user := tab(find({\textquotedbl}@{\textquotedbl})) then move(1) \\
\>   \ \ \ tab(0) \\
\>   \ \ \ \} \\
\>   net :=
...
The next part of \texttt{italk}{\textquotesingle}s code to consider is
...
{\textbar}{\textbar} lines, {\textquotedbl}columns=80{\textquotedbl}) \\
