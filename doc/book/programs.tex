...
languages. Detailed instructions show how to \index{compile}compile
and \index{run}run programs. Soon the examples introduce
...
Your \index{screen}screen should look
...
\texttt{\~{}==} stands for {\textquotedbl}\index{not equals}not
...
and the \texttt{while} loop, which test for \index{success}success.
...
\index{every}\texttt{every} processes the entire
...
of \index{iterator}\textit{iterator}\textit{s} found in other languages.
...
expression}\texttt{case} expression replaces these chains of
...
expression}\texttt{break} expression:
...
\index{scope,}\textit{scope} of parameters is limited to the body of a single
...
\>   if n = 1 then write({\textquotedbl}Move disk from {\textquotedbl},
