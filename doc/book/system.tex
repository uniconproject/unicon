...
\texttt{gre}; any integer from 0 to 255 is also accepted. The first
...
before decoding the upper layer. The optional \texttt{hdrincl=yes}
...
is in flight fails. \texttt{Attrib(h, "type")} reports the material
role (\texttt{symkey}, \texttt{key}, \texttt{cert}, \ldots).
Digest checkpoint \texttt{state}~/~\texttt{state=} is not available
with OpenSSL~3 (the MD\_CTX export APIs were removed).
