...
or third parameter to {\tt eventmask()}.

\medskip\noindent{\sffamily\bfseries \bf Note:}\ {\sffamily No value mask filtering
is performed for event codes that are not key in the value mask.}\medskip

\medskip\noindent{\sffamily\bfseries \bf Note:}\ {\sffamily Also
value masks persist across calls to {\tt EvGet()}. They are replaced
when a new value mask is supplied, or disabled if a non-table is
passed as the value mask parameter.}\medskip
...
{\bf procedure}  & {\bf returns or computes} \\\hline
