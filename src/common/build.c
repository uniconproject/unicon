...
void check_Selections(int current)
{
    ...
    else if (current == _3D_Fonts_){ /* Check for the 3D Graphics Configurations */
...
void disableFeatures(FILE *fout, struct Info List[], int max, int type)
{
    ...
    fprintf(fout,"\n");
    fprintf(fout,"/*\n");
    fprintf(fout," * The following is disabled by the USER \n");
    fprintf(fout," */\n");
    for(i=0; i < max; ++i)
	fprintf(fout,"%s\n",List[i].undefined);
    fprintf(fout,"\n");
    ...
    if ( type == _ODBC_ ){
	    ...
	    if ((!strcmp(List[0].lib1,_AC_LIBS[i])) ||
                ((!strcmp(List[1].lib1,_AC_LIBS[i]) || (!strcmp(List[1].lib2,_AC_LIBS[i])))))
                _AC_LIBS[i][0] = '\0';
    }
...
/*
 * Reads Makedefs and stripes it from the unwanted libraries
