...
EOF has been seen.
For a cryptographic handle, \texttt{Attrib(h, "type")} reports the
material role; assignments such as \texttt{op=}, \texttt{alg=},
\texttt{cipher=}, \texttt{iv=}, and \texttt{sig=} are allowed only in
the idle window between operations. Digest checkpoint
\texttt{Attrib(h, "state")}~/~\texttt{Attrib(h, "state=\textit{blob}")}
is not supported with OpenSSL~3.
...
/ \texttt{PacketSpec} in Chapter~5). The same attributes can be read
or changed after \texttt{open()} with \texttt{Attrib()} (except
\texttt{proto}, which is fixed at \texttt{socket()} time). See
