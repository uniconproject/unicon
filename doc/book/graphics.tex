...
\>repeat if Event() === (\&ldrag {\textbar} \&mdrag {\textbar} \&rdrag) then \{\\
\>\>\# erase box at old position, then draw at new position\\
\>\>FillRectangle({\textbackslash}x, {\textbackslash}y, 10, 10)\\
...
\iconcode{
[\textit{lightness}] [\textit{saturation}] [\textit{hue}[ish]] \textit{hue}
}
...
\>\>i +:= 1 \\
\>\>argv[2] ? \{ \\
\>\>\>width := integer(tab(many(\&digits))) {\textbar} stop("bad -size") \\
\>\>\>="," {\textbar} stop("bad -size") \\
\>\>\>height := integer(tab(0)) {\textbar} stop("bad -size") \\
\>\>\>i +:= 1 \\
\>\>\>\} \\
\>\>\}
}
...
\iconcode{
\>\>\>else \{ \# x {\textless} logical 0. User clicked in mouse area \\
...
\>\>\>\>until Event() === (\&mrelease {\textbar} \&lrelease{\textbar} \&rrelease) \\
\>\>\>\>\} \\
\>\>\>\}
}
...
\>if s := argv[i] then \{ \\
\>\>\&window := open("pme", "g", "image=" {\textbar}{\textbar} s) {\textbar}
stop("cannot open window") \\
\>\>width {\textless}:= WAttrib("width") \\
\>\>height {\textless}:= WAttrib("height") \\
\>\>lmargin := max(width, 65) \\
...
\>\>\} \\
\>else \{ \\
\>\>\&window := open("pme", "g", "size=" {\textbar}{\textbar}
...
\iconcode{
\>[ diffuse {\textbar} ambient {\textbar} specular {\textbar} emission] \textit{color name}
}
...
\iconcode{
\>Fg(w, \textrm{"}diffuse light grey; ambient grey; specular black; emission black; \_ \\
\>\> shininess 50\textrm{"})
}
...
\medskip\noindent{\sffamily\bfseries \bf Note:}\ {\sffamily In the
