...
\iconcode{
procedure main() \\
\>write("Type a line ending with {\textless}ENTER{\textgreater}:") \\
...
and the result is real, so 8.0/3 is 2.66666...}\medskip
...
\begin{flushleft}
\tablehead{}
\begin{xtabular}{m{0.75cm}m{0.75cm}m{0.75cm}m{0.75cm}m{0.75cm}m{0.75cm}m{7.00cm}}
\texttt{{\textless}} & \texttt{{\textless}=} & \texttt{{\textgreater}} &
\texttt{{\textgreater}=}  & \texttt{=} & \texttt{\~{}=} &
\index{numeric comparison}numeric comparison operators \\
\texttt{{\textless}{\textless}} & \texttt{{\textless}{\textless}=} &
\texttt{{\textgreater}{\textgreater}} & \texttt{{\textgreater}{\textgreater}=} & \texttt{==} &
\texttt{\~{}==} & \index{lexical comparison}lexical (alphabetic) comparison \\
 & \texttt{===} & & \texttt{\~{}===} & & & \index{reference comparison}reference comparison \\
\end{xtabular}
\end{flushleft}
...
\iconcode{
\>(x == 3) {\textbar}{\textbar} (x == 5) {\textbar}{\textbar} (y == 3) {\textbar}{\textbar} (y == 5)
}
...
\iconcode{
\>every f(1 {\textbar} 4 {\textbar} 9 {\textbar} 16 {\textbar} 25 {\textbar} 36)
}
...
\iconcode{
\>write("The value of x is ", {\textbackslash}x)
}
...
\medskip\noindent{\sffamily\bfseries \bf Warning:}
{\sffamily
...
to try; \texttt{squares()} generates an infinite result sequence.)}\medskip
