...
char set_child_window(wbp wb2, wbp wb, int child_window)
{
   wsp ws = wb->window;
   wsp ws2 = wb2->window;
   Display *display;
...
/*      *err_index = -2; */
   ...
   ws->x += w->context->dx;
#ifdef Graphics3D
   w->context->is_3D = ws->is_3D = is_3d;
#endif					/* Graphics3D */
   return 1;
}
      ...
      if (ws->is_3D) {
         attr.colormap = 
   		        XCreateColormap(wd->display,
                        wp->window->win, wd->vis->visual, AllocNone);
         ws->win = ((ws->iconic == RootState) ? DefaultRootWindow(wd->display) :
		        XCreateWindow (wd->display, wp->window->win, /*DefaultRootWindow(wd->display), */
                                  ws->posx < 0 ? 0 : ws->posx, 
				            ws->posy < 0 ? 0 : ws->posy, 
           ...
           ws->win = ((ws->iconic == RootState) ? DefaultRootWindow(wd->display) :
		       ...
		       XCreateSimpleWindow(wd->display, wp->window->win, /*DefaultRootWindow(stddpy), */
				     ws->posx < 0 ? 0 : ws->posx,
...
*/
   ...
   if (child_window==2) {
     ...
     w->context->texName = malloc(16 * sizeof(GLuint));
     w->context->nalced = 16;
     w->context->ntextures = 0;
     w->context->curtexture = -1;
...
*/     
     ...
     glGenTextures(16, w->context->texName);
