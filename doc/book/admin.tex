...
\>   f := open(scriptfile, flags) {\textbar}
		stop("Couldn't
open ", image(scriptfile)) \\
\>   tty := open("/dev/tty"
{\textbar}{\textbar} name, "rw")
{\textbar} stop("Couldn't
open tty!") \\
...
\>   \ \ \ \ \ \ write(\&errout,
"Couldn't stat ",
dir{\textbar}{\textbar}name, " ", sys\_errstr(\&errno)) \\
\>   \ \ \ \ \ \ next \\
\>   \ \ \ \} \\
...
"@" {\textbar}{\textbar} server \\
 \ \ s := open(url, "m") {\textbar} stop("Couldn't connect to
", image(url)) \\
...
\\
 ...
 \ \ \ \ \ stop("Couldn't read filter
rules from ", image(args[1]))\\
