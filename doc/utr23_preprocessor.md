...
2. If there are any object-like or function-like macros, scan the line for:
   - **`#`** outside strings → treat as start of comment: position to end (`tab(0)`), stopping macro scan (Unicon `#` comment).
...
- **`preproc_istring()` / `preproc_istring_radix()`** — Decode Unicon escapes in quoted filenames and similar.
...
**`$define` body continuation.** The replacement may continue on following lines if each continued line ends with **`\`** as the **very last character** on the line (C preprocessor splice; no trim). The scanner uses **`preproc_define_value_next_line()`** together with **`preproc_scan_define_value()`**; leading space is skipped only before the first line of the body. The same rule applies to **object-like** **`$define`** values.
...
**`&line` / `#line`.** Macro-**call** continuation anchors **`preproc_print_line`** to the first physical line of the call so **`&line`** matches that line and following statements sync past continuation rows. **`$define` `\`** splicing does not adjust **`preproc_print_line`** (definitions emit no tokens). **`&line`** inside expanded code generally reflects the **invocation** site, not each physical line of a multiline **`$define`** body. See **`macros_multiline_line.icn`**, **`macros_define_multiline_line.icn`**.
...
- **`$define` body.** **`preproc_scan_define_value()`** collects text to end-of-line rules; a trailing **`\`** (very last character on the line; no trim) splices the next physical line via **`preproc_define_value_next_line()`**, which appends to **`&subject`**. Unlike macro-argument continuation, **`$define`** produces no output tokens, so **`preproc_print_line`** is **not** adjusted there; **`&line`** at call sites still reflects the **invocation** line after **`\`** splicing.
...
|------|--------|------------|
| **Debugging** | **`$define __debug__`** or **`-D__debug__`** | Diagnostic output, then **`runerr(219, …)`** for both **`assert`** and **`assert_not`** (**`assert_not`** prefixes the value with **`char(29)`**) — see **`src/runtime/data.r`**, **`errmsg.r`**, **`rdebug.r`**. |
...
- **Modes.** If **`__test__`** is enabled (**`preproc_sym_enabled`**, including the implicit interaction with **`__debug__`** from **`preproc_new`** when **`__test__`** is defined), the expansion uses **`write`** + **`fail`**. If only **`__debug__`**, it uses **`runerr(219, …)`** (with a **`char(29)`** prefix for **`assert_not`**). If neither symbol is set, **`expanded`** is **`""`**—the call disappears entirely (**no tokens**).
...
Most work uses Unicon **string scanning** on the current line: **`line ? { … }`** sets **`&subject`** to `line` and **`&pos`** to the cursor. Directives read tokens with **`preproc_word()`**, **`preproc_qword()`**, etc. Text expansion walks the same string, mutating **`&subject`** when triple-quoted regions are rewritten in place (`preproc_rewrite_triple_strings()`). (Unicon **pattern matching** is a separate language feature and is not used here.)
...
**Staleness:** that section describes **object-like** macros only (“macros do not have parameters”, “600+ line file”). The current `preproce.icn` is larger and includes function-like macros, built-in assertions, and triple-quoted strings; treat **§7** of this UTR as the up-to-date description of those features.
