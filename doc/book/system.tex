...
\index{input}input/\index{output}output interactions with the
...
\index{reference!file}reference to resources allocated by
...
functions \index{read()}\texttt{read()} and
\index{write()}\texttt{write()} are often used. These functions by
default use special files denoted by the keywords \index{input,
standard \&input}\texttt{\&input} and \index{output, standard
\&output}\texttt{\&output}, respectively. There is a third file
keyword, \index{error!standard, \&errout}\texttt{\&errout}, that refers
...
portable fields are \texttt{size}, the \index{file size}file size in
...
\texttt{chown(fname, user, group)}. The \index{chown()}\texttt{chown()}
...
\index{chmod()}\texttt{chmod()} function only succeeds for the owner of a
...
Files can be \index{file!lock}locked while a program is
...
from \index{trap()}\texttt{trap()} so it can be restored by a
...
\texttt{{\textquotedbl}p{\textquotedbl}} (\index{pipe}pipe) to
...
calls \index{system()}\texttt{system()} normally waits for the launched
...
\> handle\_timeout() \\
(\&errno = 0) {\textbar} stop({\textquotedbl}Select
 \index{expression failure}failed: {\textquotedbl}, \&errortext) \\
every f := !L do \{ \\
...
\>   argv[1] ? \{ \\
\>   \ \ \ name := tab(find({\textquotedbl}@{\textquotedbl})) \&
...
\iconcode{
procedure main(argv) \\
\>   n := open(argv[1],{\textquotedbl}n{\textquotedbl}) | \\
\>\>      stop({\textquotedbl}can't connect to {\textquotedbl}, argv[1]|{\textquotedbl}missing host{\textquotedbl}) \\
\>   write({\textquotedbl}connected to: {\textquotedbl}, image(n)[6:-1]) \\
\>   write({\textquotedbl}using: {\textquotedbl}, gethost(n)) \\
end
}
...
\textstyleSourceText{\texttt{f1}}, \textstyleSourceText{\texttt{f2}},
and \textstyleSourceText{\texttt{f3}}\textstyleSourceText{ }are file
arguments to use as \textstyleSourceText{\texttt{\&input}},
\textstyleSourceText{\texttt{\&output}}, and
\textstyleSourceText{\texttt{\&error}} in the loaded task;
\textstyleSourceText{\texttt{\&input}},
\textstyleSourceText{\texttt{\&output}}, and
\textstyleSourceText{\texttt{\&error}}\textstyleSourceText{\textrm{
}}default to those of the loading task.\textstyleSourceText{\textrm{
}}\textstyleSourceText{\texttt{i1,}} \textstyleSourceText{\texttt{i2}},
and \textstyleSourceText{\texttt{i3}} are three integer arguments that
supply initial region sizes for the task{\textquotesingle}s block,
string, and stack memory areas, respectively.
\textstyleSourceText{\texttt{i1}} and \textstyleSourceText{\texttt{i2}}
default to 65000, while \textstyleSourceText{\texttt{i3 }}defaults to
20000 (the defaults may be changed by the environment variables
\textstyleSourceText{\texttt{BLKSIZE}},
\textstyleSourceText{\texttt{STRSIZ}}\texttt{E}, and
\textstyleSourceText{\texttt{MSTKSIZE}}). 

\subsubsection{Running other programs}

A co-expression created by \textstyleSourceText{\texttt{load()}} is
activated like any other co-expression. When activated with the
\textstyleSourceText{\texttt{@}} operator, the child task begins
executing its main procedure. Unless it suspends or activates
\textstyleSourceText{\texttt{\&source}}, the child task runs to
...
\textstyleSourceText{\texttt{seqload}}, which loads and executes each
...
\textstyleSourceText{\texttt{load()}}\texttt{.
}\textstyleSourceText{\texttt{load()}} reads the code for each argument
...
\textstyleSourceText{\texttt{@}}. This is ordinary Icon code; there is
nothing special about this example except the semantics of the
\textstyleSourceText{\texttt{load()}} function and the independent
execution environment (separate global variables, heaps, and so forth),
that \textstyleSourceText{\texttt{load()}}\textstyleSourceText{
...
\iconcode{
\textstyleSourceText{\texttt{seqload translate assemble link}}}
...
to task through
\textstyleSourceText{\texttt{main()}}{\textquotesingle}s argument list,
...
named \textstyleSourceText{\texttt{calc()}} and a main procedure that
exports a reference to \textstyleSourceText{\texttt{calc()}} for
sharing: 
...
a procedural encapsulation (\textstyleSourceText{\texttt{loadlib()}} in
...
\textstyleSourceText{\texttt{load()}} function is called and the shared
library activated. 
...
A client of \textstyleSourceText{\texttt{calc}} declares a global
