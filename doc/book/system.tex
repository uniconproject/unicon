...
\begin{itemize}\itemsep0pt
  ...
  \item Write \index{Internet}Internet \index{client}client and
    \index{server}server applications, including multicast and
    raw IP sockets
  \item Hash, sign, and encrypt data with OpenSSL-backed crypto handles
  \item Create Linux \index{network namespace}network namespaces for
    isolated test topologies
\end{itemize}
...
The record returned by \texttt{receive()} is
\texttt{posix\_message}: \texttt{addr} is the sender in
\texttt{"}\texttt{\textit{host}}\texttt{:}\texttt{\textit{port}}\texttt{"}
form, \texttt{msg} is the payload. \texttt{saddr} and
\texttt{daddr} are the IP-header names for source and destination.
On UDP, \texttt{saddr} is the numeric peer address and
\texttt{daddr} is null. On an IPv4 raw receive that includes a
header, \texttt{saddr}/\texttt{daddr}/\texttt{ttl}/\texttt{proto}
...
After \texttt{open()}, assignments still use
\index{Attrib()}\texttt{Attrib()} (\texttt{"\textit{name}=\textit{value}"}).
Status is a non-destructive peek: \texttt{f["ttl"]}, \texttt{f["iface"]},
\texttt{f["groups"]}, \texttt{f["proto"]}, \texttt{f["hdrincl"]}.
\texttt{f["groups"]} is a list of joined group names (SSM as
\texttt{source@group}); generate members with \texttt{!f["groups"]}.
\texttt{key(f)} generates answerable field names.
...
when the membership was joined on a specific interface.
\texttt{f["groups"]} reports joined groups as a list;
\texttt{join}/\texttt{leave}
themselves are not peek fields.

\subsection*{Raw Sockets}
\index{raw socket}\index{SOCK\_RAW}
Mode \texttt{"nr"} opens an IP-level \index{raw IP}raw socket
(\texttt{SOCK\_RAW}). Unlike TCP and UDP, the application supplies
(and receives) upper-layer protocol bytes --- ICMP, IGMP, OSPF, PIM,
and so on --- while the kernel still builds the IP header unless
header inclusion is requested. Raw sockets usually require elevated
privileges (root or administrator).

The \texttt{proto=} attribute is required and selects the IP protocol
number passed to \texttt{socket()}. Symbolic names include
\texttt{icmp}, \texttt{igmp}, \texttt{ospf}, \texttt{pim}, and
\texttt{gre}; any integer from 0 to 255 is also accepted.
\texttt{f["proto"]} peeks that number; \texttt{Attrib()} cannot
change it. The first
argument to \texttt{open()} is a bare host or address (no
\texttt{:port}): raw destinations are hosts, not transport endpoints.
IPv6 literals are accepted as hosts and are not misparsed as
\texttt{host:port}.

\iconcode{
\>   f := open("8.8.8.8", "nr", "proto=icmp", "ttl=64") {\textbar} \\
\>\>\>\>stop(\&errortext) \\
\>   writes(f, icmp\_bytes) \\
\>   r := receive(f) \\
\>   write(r.addr, "  ", *r.msg, " bytes")
}

On many platforms a raw receive buffer includes the IPv4 header in
front of the protocol message; strip it (or use a library helper)
before decoding the upper layer. When that header is present,
\texttt{r.saddr}, \texttt{r.daddr}, \texttt{r.ttl}, and
\texttt{r.proto} are the source, destination, hop limit, and
protocol from it. The optional \texttt{hdrincl=yes}
attribute sets \texttt{IP\_HDRINCL} so writes may supply a complete
IP header as well --- useful for crafted packets, but not required
for ordinary ICMP or IGMP.

The runtime does not build protocol payloads. Use
\texttt{import~net} and the \texttt{PacketSpec} layouts in
\texttt{uni/lib} (see below), or assemble the bytes yourself.

\subsection*{PacketSpec and package net}
\index{PacketSpec}\index{package net}\index{ICMP}\index{IGMP}
The Unicon library module \texttt{packetspec} (package \texttt{net})
provides a small declarative engine for fixed binary layouts:
\texttt{field()}, \texttt{ipv4\_field()}, \texttt{checksum\_field()},
\texttt{set()}/\texttt{build()}/\texttt{decode()}, and
\texttt{describe()} for REPL discovery. Layer-1 helpers pack and
unpack network-order integers (\texttt{u8}/\texttt{u16}/\texttt{u32}/
\texttt{u64}) and IPv4 addresses (\texttt{ipv4}/\texttt{ipv4\_raw}/
\texttt{to\_ipv4}/\texttt{get\_ipv4}). Internet checksums use
\texttt{inet\_cksum()}; \texttt{checksum\_field()} may exclude a
byte range from the sum. Fields marked with \texttt{require()}
must be supplied before \texttt{build()}, unless
\texttt{relax()} or \texttt{build(..., lax)} is used (for
crafting incomplete test packets).

Protocol classes inherit \texttt{PacketSpec} and override
\texttt{define()} to declare their header. The library currently
includes ICMP echo (\texttt{icmp.icn}) and IGMPv2/v3
(\texttt{igmp.icn}):

\begin{description}
\item[\texttt{IcmpEchoRequest} / \texttt{IcmpEchoReply}]
\texttt{echo(id, seq, payload)} or chainable
\texttt{set("id",~…)\allowbreak.set("seq",~…)\allowbreak.build()};
\texttt{match\_echo(pkt, id, seq)} succeeds on a matching type~0
reply after stripping the IPv4 header from a raw receive buffer.
\item[\texttt{Ping}]
Session wrapper: \texttt{Ping(host)} opens
\texttt{proto=icmp}, and \texttt{echo()} sends a request and waits
for a matching reply (returns a table with \texttt{rtt} and
\texttt{addr}).
\item[\texttt{Igmpv2Query} / \texttt{Igmpv2Report} / \texttt{Igmpv2Leave}]
Eight-octet IGMPv2 messages (RFC~2236); report/leave require
\texttt{group}.
\item[\texttt{Igmpv3Query} / \texttt{Igmpv3Report}]
IGMPv3 queries with source lists and membership reports with group
records (RFC~3376), including convenience methods
\texttt{join}/\texttt{leave}/\texttt{join\_sources} for ASM and SSM.
\item[\texttt{Igmp}]
Session wrapper: \texttt{Igmp()} opens \texttt{proto=igmp};
\texttt{recv()} returns a classified table (\texttt{kind},
\texttt{addr}, \ldots); \texttt{report}/\texttt{leave}/\texttt{join}
send common messages.
\end{description}

A minimal ICMP echo with the session wrapper:
...
procedure main() \\
\>   local p, v \\
\>   p := Ping("8.8.8.8") {\textbar} stop(\&errortext) \\
\>   write(p.describe()) \\
\>   if v := p.echo() then \\
...
The same exchange with an explicit raw socket and PacketSpec:
...
\>   req := IcmpEchoRequest() \\
\>   reply := IcmpEchoReply() \\
\>   id := iand(?100000 {\textbar} 1, 16rFFFF) \\
\>   f := open("8.8.8.8", "nr", "proto=icmp", "ttl=64") {\textbar} \\
\>\>\>\>stop(\&errortext) \\
\>   pkt := req.set("id", id).set("seq", 1).build(, "unicon") \\
\>   writes(f, pkt) \\
\>   r := receive(f) \\
\>   if reply.match\_echo(r.msg, id, 1) then \\
\>\>\>write("reply from ", r.addr) \\
\>   close(f) \\
end
}

Ready-made programs in \texttt{uni/progs} exercise the same path:
\texttt{uping} is an ICMP ping client, and \texttt{uigmp} is a raw
IGMP sniffer. Both need privileges for \texttt{SOCK\_RAW}. See
Chapter~14 for short usage notes.

\subsection*{Network Namespaces}

On Linux, when the feature string \texttt{Network namespaces} is
present, mode \texttt{"j"} creates a \index{network namespace}network
namespace: an isolated network stack for building lightweight test
topologies. The value returned by \texttt{open()} is a file handle,
not a stream for reading or writing. Closing it tears down the
namespace's filesystem pins. An optional leading namespace file to
\texttt{fork()}, \texttt{spawn()}, or \texttt{system()} runs the
child inside that namespace. Status is peeked with
\texttt{ns["name"]}, \texttt{ns["persist"]}, \texttt{ns["userns"]},
and \texttt{ns["refcount"]}; \texttt{key(ns)} generates those names.
Boolean peeks succeed with \texttt{"yes"} or \texttt{\&null}.

\iconcode{
procedure main() \\
\>   ns := open("r1", "j") {\textbar} stop(\&errortext) \\
\>   write(ns["name"], " userns=", \ns["userns"] {\textbar} "no") \\
\>   system(ns, ["ip", "link", "show"]) \\
\>   close(ns) \\
end
}

Trailing attributes may include \texttt{persist=yes} (listable by
\texttt{ip netns}), \texttt{bridge=yes}, and \texttt{userns=yes} for
unprivileged bootstrap. Higher-level link helpers are in
\texttt{package net} (\texttt{uni/lib/netns.icn}); see
\texttt{uni/progs/unettopo.icn} and Chapter~14.
...
Mode \texttt{"ne-"} or \texttt{verifyPeer=no} skips peer certificate
verification. \texttt{keypass=} unlocks an encrypted private key.
...
\iconcode{
procedure main() \\
\>   s := open("user@router.example", "hc", "key=~/.ssh/id\_rsa", \\
...
command; requires mode \texttt{"hc"}), and \texttt{channel=} (\texttt{yes}/\texttt{no}). Interactive
...
running agent are tried. Mode \texttt{"h-"} or \texttt{verifyPeer=no}
skips host-key verification (\texttt{verifyPeer=} overrides the mode
flag, as on TLS).
Without \texttt{cmd=}, mode \texttt{"h"} opens an interactive shell on a PTY.
...
(useful for SFTP or later \texttt{open(s, "hc")} / \texttt{open(s, "hs")} handles);
\texttt{channel=yes} is the default. Combining \texttt{channel=no} with
mode \texttt{"hc"} or \texttt{cmd=} fails. The default port is 22 when \texttt{:port} is
omitted.

A second \texttt{open(s, ...)} on an existing SSH file opens another
channel or SFTP handle on the same session (no new handshake). The
second argument is a mode string: \texttt{"hc"} for an extra
shell or exec channel (\texttt{cmd=} selects exec), \texttt{"hs"} for
SFTP. Closing the session invalidates every handle derived from it.
...
\texttt{c["exitstatus"]}, \texttt{c["stderr"]}, and
\texttt{c["eof"]} report the command's exit status, the current
accumulated stderr (a peek, not a drain), and whether remote EOF has arrived
(\texttt{"yes"} or \texttt{\&null}).
\texttt{key(c)} generates answerable field names.
On a session, \texttt{s["authmethods"]} is a list of advertised
method names; \texttt{s["kex"]} is the key-exchange algorithm.
...
SFTP uses the same session. Mode \texttt{"hs"} with
...
\>   c := open(s, "hs", "path=/tmp/out.bin", "w") {\textbar} stop(\&errortext) \\
...
\subsection*{Cryptographic Facilities}

When Unicon is built with OpenSSL (already required for secure sockets),
modes \texttt{"e"}, \texttt{"er"}, and \texttt{"eh"} also open
\index{cryptography}cryptographic handles for hashing, HMAC, signing,
verification, and symmetric encryption. These are distinct from TLS
socket mode \texttt{"ne"}: the first argument is an algorithm name, a
key or certificate path, a raw key string (\texttt{"er"}), or another
crypto handle that supplies material. Trailing
\texttt{"\textit{name}=\textit{value}"} attributes select the operation
and parameters. Failures set \texttt{\&errortext} and do not abort.

\iconcode{
import crypto\\
\\
procedure main() \\
\>   h := open("sha256", "eh") {\textbar} stop(\&errortext) \\
\>   write(h, "abc") \\
\>   write("digest=", hexbytes(read(h), 1)) \\
\>   close(h) \\
end
}

Mode \texttt{"eh"} hashes; the target is the digest name
(\texttt{sha256}, \texttt{sha512}, \ldots). Writes accumulate input;
\texttt{read()} finalizes and resets so the handle can be reused.
HMAC and public-key operations use mode \texttt{"e"} (or
\texttt{"er"} for an inline symmetric key) with \texttt{op=}:

\iconcode{
\>   k := open("secret-key-bytes", "er") \\
\>   m := open(k, "e", "op=hmac", "alg=sha256") \\
\>   write(m, "message") \\
\>   mac := read(m) \\
\>   close(m); close(k)
}
...
\iconcode{
\>   key := open("client.key", "e") \\
\>   cert := open("client.crt", "e") \\
\>   signer := open(key, "e", "op=sign", "alg=sha256") \\
\>   verifier := open(cert, "e", "op=verify", "alg=sha256") \\
\>   write(signer, "payload"); sig := read(signer) \\
\>   Attrib(verifier, "sig=" || sig) \\
\>   write(verifier, "payload") \\
\>   write(if read(verifier) then "ok" else "fail")
}

Symmetric encrypt/decrypt defaults to \texttt{aes-256-gcm}. With no
\texttt{iv=}, a fresh IV is chosen per message and prefixed on the
ciphertext; an explicit \texttt{iv=} omits that prefix. Package
\texttt{crypto} (Unicon class library) supplies \texttt{hexencode}~/
\texttt{hexbytes}, \texttt{hexdecode}, and base64 helpers for dumping
binary digests and keys.

Modes \texttt{"we"} and \texttt{"re"} apply the same operations to a
named file: writes encrypt (finalized on \texttt{close()}), reads
decrypt or hash. The key is given with \texttt{key=} or a material
handle attribute rather than as the open target.

\iconcode{
\>   f := open("secrets.dat", "we", "op=encrypt", "key=aes.key") \\
\>   writes(f, plaintext); close(f) \\
\>   f := open("secrets.dat", "re", "op=decrypt", "key=aes.key") \\
\>   while chunk := reads(f, 4096) do put(pending, chunk) \\
\>   if \textbackslash\&errornumber then \{ /* AEAD auth failed */ \}
}

Encrypted TCP remains mode \texttt{"ne"}; encrypted UDP (DTLS) uses
\texttt{"nue"}~/~\texttt{"naue"}. Material handles from mode
\texttt{"e"} may be passed as attributes so certs and keys are parsed
...
After a handshake, \texttt{conn["cipher"]} peeks the negotiated
suite and \texttt{conn["version"]} the TLS version;
\texttt{conn["certchain"]} is a list of peer certificates
as PEM strings and \texttt{conn["san"]} is a list of peer
Subject Alternative Names.

Useful attributes include \texttt{op=} (\texttt{hash}, \texttt{hmac},
\texttt{sign}, \texttt{verify}, \texttt{encrypt}, \texttt{decrypt}),
\texttt{alg=} (digest name), \texttt{cipher=}, \texttt{iv=},
\texttt{sig=} (for verify), and \texttt{keypass=} (key-file passphrase;
\texttt{password=} is an alias). Mode \texttt{"ne-"} is the same as
\texttt{verifyPeer=no}. \texttt{Attrib()} may change them only in
the idle window between operations; a change while a digest or cipher
is in flight fails. \texttt{h["type"]} reports the material
role (\texttt{symkey}, \texttt{key}, \texttt{cert}, \ldots);
\texttt{h["op"]} reports the operation (\texttt{hash}, \texttt{hmac},
\texttt{sign}, \texttt{verify}, \texttt{encrypt}, \texttt{decrypt});
\texttt{h["subject"]}, \texttt{h["san"]}, \texttt{h["notafter"]}, and
\texttt{h["expired"]} introspect a certificate. On a hash, HMAC, or
sign handle, \texttt{h["hash"]} peeks
the current digest, MAC, or signature without finalizing. On encrypt/decrypt,
\texttt{h["iv"]} peeks the IV that was set or generated.
\texttt{key(h)} generates answerable field names.
